...
struct are_same
{
    static constexpr bool value = std::conjunction_v<
        std::is_same<remove_cvref_t<T>, remove_cvref_t<Ts>>...>;
};
...
struct is
{
    ...
    static_assert(are_same_v<BaseType, Ts...>,
    ...
    constexpr bool compare(const BaseType& min,
                           const BaseType& max,
    ...
    template<std::size_t... I>
    constexpr bool compare_equal(const BaseType& value,
                                 std::index_sequence<I...>) const
    {
    ...
    constexpr bool operator==(const BaseType& value) const
    {
        return equal(value);
    }

    friend constexpr bool operator==(const BaseType& lhs, const is& rhs)
    {
        return rhs == lhs;
    }
};
